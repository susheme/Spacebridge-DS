...
(() => {
  ...
  sbRegister({
    ...
    description: sbT(
      ...
      + '<b>Alignment:</b>'
      + '<ul><li><code>stretch</code> only stretches children whose height is not set. Components in this system usually fix theirs — a Button is 40px, a Search field is 32px — so nothing stretches and the shorter one simply sits at the top of the row;</li>'
      + '<li>for a row of differently sized controls use <code>center</code>. That is what the Tool Bar does.</li></ul>'
      + '<b>Adaptivity:</b>'
      ...
      + '<li><code>sbMkPage({ content, width })</code> — width: <code>default</code> / <code>text</code>.</li></ul>'
      + '<b>Выравнивание:</b>'
      + '<ul><li><code>stretch</code> растягивает только тех детей, у кого высота не задана. У компонентов системы она обычно фиксирована — кнопка 40px, поле поиска 32px — поэтому растягивать нечего, и более низкий элемент просто встаёт по верху ряда;</li>'
      + '<li>для ряда из разных по высоте контролов берите <code>center</code>. Так делает Tool Bar.</li></ul>'
      + '<b>Адаптивность:</b>'
