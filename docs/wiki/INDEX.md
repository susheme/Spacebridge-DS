...
4. Знания, которые УЖЕ ведёт репо, в вики не дублируются, на них ссылаются: релизы — `CHANGELOG.md`, отложенное и известные баги — `BACKLOG.md`, **версионирование и правила для людей — `CONTRIBUTING.md`** (версия DS — только бейдж Nav Bar в `index.html`), догма с разбором кейса — `js/components/CLAUDE.md`, история компонента — `git log --oneline --follow -- js/components/<name>.js`.
