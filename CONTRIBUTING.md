...
Обновил экспорт переменных в `Figma Tokens/*.json` — прогони:

```
/System/Library/Frameworks/JavaScriptCore.framework/Versions/A/Helpers/jsc tools/gen-tokens.js
```

`css/tokens.css` соберётся сам. Руками его значения не правь — перезапишутся.

Три режима Figma (Desktop / Tablet / Mobile) раскладываются в media-запросы:
база — мобильные значения, дальше переопределения. Компоненты менять не нужно,
адаптивность включается сама.

Завёл новую переменную, а генератор ругается «НЕ СОПОСТАВЛЕНО» — значит её имя
в Figma не совпало с именем токена в CSS. Допиши пару в `tools/token-map.json`.
...
Запускать после правок — особенно если трогал CSS (тест сверяет, что копия
стилей в JS не отстала от `.css`-файла) или порядок `<script>` в `index.html`.

Проверяется разметка и связи, но НЕ внешний вид: пустоты, обрезки и налезания
тест не увидит — это по-прежнему на глаз.
