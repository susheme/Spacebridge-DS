...
- С 15.09.2026 страницы подключают ПОЛНЫЙ JS-рантайм DS (скрипты в порядке index.html, кроме init.js и updates-ui.js — они строят хром доксайта): попапы, коллапс actions хэдэров, глазок пароля — штатные. SHELL_JS поверх — только тема и переходы между страницами (override sbSelectNavBtn с вызовом оригинала).
