...
- ❌ **Не верстай чужой компонент разметкой** (`class="sb-X"` руками в потребителе) — только его `sbMk*`-фабрика. Фабрики нет или она не экспортирована → СТОП, спроси разрешение на экспорт, не копируй. Копипаста разметки = нарушение догмы Клементия; момент, когда рука набирает чужой `class="sb-…"` — и есть момент нарушения.
